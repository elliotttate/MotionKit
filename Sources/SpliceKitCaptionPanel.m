...
- (NSDictionary *)addCaptionTitlesDirectlyToTimeline {
    ...
    // Builds FCPXML with titles at absolute offsets in a spine, imports via
    ...
    // ---------------------------------------------------------------
    // Step 1: Build FCPXML with titles in a flat spine at absolute offsets.
    ...
    if (useWordProgress) {
        for (SpliceKitCaptionSegment *seg in self.mutableSegments) {
            [xml appendString:[self wordHighlightTitlesForSegment:seg
                                                        tsCounter:&tsCounter
                                                           indent:ind
                                                             lane:nil]];
            titleCount += (int)seg.words.count;
        }
    } else {
        for (SpliceKitCaptionSegment *seg in self.mutableSegments) {
